...
import { getPracticeOverview } from "@/lib/server/practice-overview";
...
export async function GET(request: Request) {
  ...
  try {
    const result = await getPracticeOverview(auth.user.id);
    return NextResponse.json(result, { headers: response.headers });
  } catch (error) {
    return createApiErrorResponse(error, "Unable to load practice overview.", response.headers);
  }
}
